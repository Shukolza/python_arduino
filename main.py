...
def send_delay():
    ...
    if delay_ms < 0:
        status_label.config(text="Задержка должна быть неотрицательной" if LANGUAGE == 'RU' else 'The delay must be non-negative')
        return
    command = (
        f"{delay_ms}\n"  # Добавляем символ новой строки для обозначения конца сообщения
    )
    arduino.write(command.encode())
    status_label.config(text=f"Отправлено: {delay_ms} мс" if LANGUAGE == 'RU' else f'{delay_ms}ms sent')
...
button_test = ttk.Button(main_window,text="Задать задержку" if LANGUAGE == 'RU' else 'Set delay', command=send_delay)
delay_entry.pack()
button_test.pack()
...
attention = ttk.Label(main_window, text="Задержку нужно вводить в миллисекундах!" if LANGUAGE == 'RU' else 'The delay must be entered in milliseconds')
attention.pack()

try:
    arduino = serial.Serial(ARDUINO_PORT, BAUDRATE, timeout=1)
    sleep(2)
    status_label.config(text="Подключено к Arduino" if LANGUAGE == 'RU' else 'Arduino connected')
except serial.SerialException:
    status_label.config(text="Не удается подключиться!" if LANGUAGE == 'RU' else 'Can not connect!')
